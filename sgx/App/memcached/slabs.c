...
void slabs_prefill_global(void)
{
    ...
    while (mem_malloced < mem_limit && (ptr = memory_allocate(len)) != NULL)
    {
        grow_slab_list(0);
...
static void *slab_rebalance_alloc(const size_t size, unsigned int id)
{
    ...
    for (x = 0; x < s_cls->perslab; x++)
    {
        ...
        if ((void *)new_it >= slab_rebal.slab_start && (void *)new_it < slab_rebal.slab_end)
        {
...
static int slab_rebalance_move(void)
{
    ...
    if (slab_rebal.completed[offset] == 0)
    {
        ...
        case MOVE_FROM_LRU:
            ...
            if ((it->exptime != 0 && it->exptime < current_time) || item_is_flushed(it))
            {
                /* Expired, don't save. */
                save_item = 0;
            }
...
int sgx_start_slab_maintenance_thread(void)
{
    int ret;
    slab_rebalance_signal = 0;
    slab_rebal.slab_start = NULL;
    ...
    if ((ret = pthread_create(&rebalance_tid, NULL,
                              e_slab_rebalance_thread, NULL)) != 0)
    {
...
/**
 * Transition into the enclave for slab rebalance.
 */
void *e_slab_rebalance_thread(void *input)
{
    ecall_start_slab_rebalance(global_eid);
}
