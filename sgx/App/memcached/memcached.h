...
#define ITEM_get_cas(i) (((i)->it_flags & ITEM_CAS) ? (i)->data->cas : (uint64_t)0)
...
#define ITEM_data(item) ((char *)&((item)->data) + (item)->nkey + 1 + (((item)->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))

#define ITEM_ntotal(item) (sizeof(struct _stritem) + (item)->nkey + 1 + (item)->nbytes + (((item)->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
 ...
 */
    ...
    {
        int offset = it->nkey + 1 + ((it->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) + ((it->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0);
        int remain = offset % 8;
        if (remain != 0)
        {
            offset += 8 - remain;
...
#else
#define ITEM_schunk(item) ((char *)&((item)->data) + (item)->nkey + 1 + (((item)->it_flags & ITEM_CFLAGS) ? sizeof(uint32_t) : 0) + (((item)->it_flags & ITEM_CAS) ? sizeof(uint64_t) : 0))
#endif
...
#define EXPTIME_TO_POSITIVE_TIME(exptime) (exptime < 0) ? REALTIME_MAXDELTA + 1 : exptime
