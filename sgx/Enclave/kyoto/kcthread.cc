 ...
 */
  void Thread::detach()
  {
    ...
    ThreadCore *core = (ThreadCore *)opq_;
    if (!core->alive)
      throw std::invalid_argument("not alive");
    core->alive = false;
    if (::pthread_detach(core->th) != 0)
 ...
 */
  bool Mutex::lock_try(double sec)
  {
    ...
    if (ecode == 0)
      return true;
    if (ecode != ETIMEDOUT)
      throw std::runtime_error("pthread_mutex_timedlock");
    return false;
#endif
*/
  }
 ...
 */
  bool SpinLock::lock_try()
  {
    ...
    if (ecode == 0)
      return true;
    if (ecode != EBUSY)
      throw std::runtime_error("pthread_spin_trylock");
    return false;
#endif
*/
  }
 ...
 */
  void RWLock::lock_writer()
  {
    ...
    if (::pthread_rwlock_wrlock(rwlock) != 0)
      throw std::runtime_error("pthread_rwlock_lock");
#endif
*/
  }
 ...
 */
  bool RWLock::lock_writer_try()
  {
    ...
    if (ecode == 0)
      return true;
    if (ecode != EBUSY)
      throw std::runtime_error("pthread_rwlock_trylock");
    return false;
#endif
*/
  }
...
#else
    _assert_(core);
    if (::pthread_spin_lock(&core->sem) != 0)
      throw std::runtime_error("pthread_spin_lock");
#endif
*/
  }
...
#else
    _assert_(core);
    if (::pthread_spin_unlock(&core->sem) != 0)
      throw std::runtime_error("pthread_spin_unlock");
#endif
*/
  }
...
#else
    _assert_(core);
    if (::pthread_spin_lock(core->sems + idx) != 0)
      throw std::runtime_error("pthread_spin_lock");
#endif
*/
  }
