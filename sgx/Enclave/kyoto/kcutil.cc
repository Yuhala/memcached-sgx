 ...
 */
  size_t memdist(const void *abuf, size_t asiz, const void *bbuf, size_t bsiz)
  {
    ...
    return asiz > UINT8MAX || bsiz > UINT8MAX ? levdist<const char, uint32_t>((const char *)abuf, asiz, (const char *)bbuf, bsiz) : levdist<const char, uint8_t>((const char *)abuf, asiz, (const char *)bbuf, bsiz);
  }
 ...
 */
  size_t strucsdist(const uint32_t *aary, size_t anum, const uint32_t *bary, size_t bnum)
  {
    ...
    return anum > UINT8MAX || bnum > UINT8MAX ? levdist<const uint32_t, uint32_t>(aary, anum, bary, bnum) : levdist<const uint32_t, uint8_t>(aary, anum, bary, bnum);
  }
