 ...
 */
     ...
     */
      bool jump(const char *kbuf, size_t ksiz)
      {
        ...
        while (off > 0)
        {
          ...
          uint32_t tpivot = db_->linear_ ? pivot : db_->fold_hash(db_->hash_record(rec.kbuf, rec.ksiz));
          if (pivot > tpivot)
          {
            delete[] rec.bbuf;
            off = rec.left;
   ...
   * Default constructor.
   */
    explicit HashDB() : mlock_(), rlock_(RLOCKSLOT), flock_(), atlock_(), error_(),
   ...
   */
                   ...
                   Error::Code code, const char *message)
    {
      ...
      if (logger_)
      {
        Logger::Kind kind = code == Error::BROKEN || code == Error::SYSTEM ? Logger::ERROR : Logger::INFO;
        if (kind & logkinds_)
   ...
   */
    bool open(const std::string &path, uint32_t mode = OWRITER | OCREATE)
    {
      ...
      if (!file_.open(path, fmode, msiz_))
      {
        ...
        log_kyoto_error("file open error", emsg, __func__);
      ...
      if (((flags_ & FOPEN) || (flags_ & FFATAL)) && !(mode & ONOREPAIR) && !(mode & ONOLOCK))
      {
        ...
        if (!file_.close())
        {
          log_kyoto_error("file close error", __func__);
          set_error(_KCCODELINE_, Error::SYSTEM, file_.error());
          return false;
        }
        if (!file_.open(path, fmode, msiz_))
        {
          log_kyoto_error("file open error", __func__);
          set_error(_KCCODELINE_, Error::SYSTEM, file_.error());
          return false;
        }
        if (!load_meta())
   ...
   */
    bool scan_parallel_impl(Visitor *visitor, size_t thnum, ProgressChecker *checker)
    {
      ...
      if (!offs.empty())
      {
        ...
        {
        public:
          explicit ThreadImpl() : db_(NULL), visitor_(NULL), checker_(NULL), allcnt_(0),
    ...
    bool read_record(Record *rec, char *rbuf)
    {
      ...
      else
      {
        if (rec->off + (int64_t)rec->rsiz > psiz_)
        {
          ...
          report(_KCCODELINE_, Logger::WARN, "psiz=%lld off=%lld rsiz=%lld fsiz=%lld"
                                             " snum=%04X",
                 (long long)psiz_, (long long)rec->off, (long long)rec->rsiz,
                 (long long)file_.size(), snum);
          return false;
        }
