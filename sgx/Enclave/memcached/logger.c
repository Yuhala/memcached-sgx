...
static sgx_thread_t logger_tid;
sgx_thread_mutex_t logger_stack_lock = SGX_THREAD_MUTEX_INITIALIZER;

pthread_key_t logger_key;

#if !defined(HAVE_GCC_64ATOMICS) && !defined(__sun)
sgx_thread_mutex_t logger_atomics_mutex = SGX_THREAD_MUTEX_INITIALIZER;
#endif
...
{
    sgx_thread_mutex_lock(&logger_stack_lock);
    ...
    if (l->next)
        l->next->prev = l;
    logger_stack_head = l;
    if (logger_stack_tail == 0)
        logger_stack_tail = l;
    logger_count++;
    sgx_thread_mutex_unlock(&logger_stack_lock);
    return;
}
...
static void logger_set_flags(void)
{
    ...
    {
        sgx_thread_mutex_lock(&l->mutex);
        l->eflags = f;
        sgx_thread_mutex_unlock(&l->mutex);
    }
    return;
}
...
logger *logger_create(void)
{
    ...
    sgx_thread_mutex_init(&l->mutex, NULL);
