...
sgx_thread_mutex_t lru_locks[POWER_LARGEST];

/* Connection lock around accepting new connections */
sgx_thread_mutex_t conn_lock = SGX_THREAD_MUTEX_INITIALIZER;

#if !defined(HAVE_GCC_ATOMICS) && !defined(__sun)
sgx_thread_mutex_t atomics_mutex = SGX_THREAD_MUTEX_INITIALIZER;
#endif

/* Lock for global stats */
static sgx_thread_mutex_t stats_lock = SGX_THREAD_MUTEX_INITIALIZER;

/* Lock to cause worker threads to hang up after being woken */
static sgx_thread_mutex_t worker_hang_lock;

/* Free list of CQ_ITEM structs */
static CQ_ITEM *cqi_freelist;
static sgx_thread_mutex_t cqi_freelist_lock;

static sgx_thread_mutex_t *item_locks;
...
static void wait_for_thread_registration(int nthreads)
{
    ...
    {
        sgx_thread_cond_wait(&init_cond, &init_lock);
    }
}
...
static void register_thread_initialized(void)
{
    ...
    sgx_thread_mutex_unlock(&init_lock);
    /* Force worker threads to pile up if someone wants us to */
    sgx_thread_mutex_lock(&worker_hang_lock);
    sgx_thread_mutex_unlock(&worker_hang_lock);
}
...
static CQ_ITEM *cq_pop(CQ *cq)
{
    ...
    sgx_thread_mutex_unlock(&cq->lock);
...
static CQ_ITEM *cqi_new(void)
{
    ...
    sgx_thread_mutex_unlock(&cqi_freelist_lock);
...
static void setup_thread(LIBEVENT_THREAD *me)
{
    ...
    if (sgx_thread_mutex_init(&me->stats.mutex, NULL) != 0)
    {
...
void STATS_LOCK()
{
    sgx_thread_mutex_lock(&stats_lock);
}

void STATS_UNLOCK()
{
    sgx_thread_mutex_unlock(&stats_lock);
}
...
void threadlocal_stats_reset(void)
{
    ...
    {
        sgx_thread_mutex_lock(&threads[ii].stats.mutex);
...
#undef X
        ...
        sgx_thread_mutex_unlock(&threads[ii].stats.mutex);
    }
}
...
void memcached_thread_init(int nthreads, void *arg)
{
    ...
    {
        sgx_thread_mutex_init(&lru_locks[i], NULL);
    }
    sgx_thread_mutex_init(&worker_hang_lock, NULL);

    sgx_thread_mutex_init(&init_lock, NULL);
    sgx_thread_cond_init(&init_cond, NULL);

    sgx_thread_mutex_init(&cqi_freelist_lock, NULL);
    cqi_freelist = NULL;
    ...
    item_locks = calloc(item_lock_count, sizeof(sgx_thread_mutex_t));
    if (!item_locks)
    {
    ...
    {
        sgx_thread_mutex_init(&item_locks[i], NULL);
    }
