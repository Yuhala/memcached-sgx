...
static sgx_thread_mutex_t lru_maintainer_lock = SGX_THREAD_MUTEX_INITIALIZER;
static sgx_thread_mutex_t cas_id_lock = SGX_THREAD_MUTEX_INITIALIZER;
static sgx_thread_mutex_t stats_sizes_lock = SGX_THREAD_MUTEX_INITIALIZER;
...
void item_stats_reset(void)
{
    ...
    {
        sgx_thread_mutex_lock(&lru_locks[i]);
        memset(&itemstats[i], 0, sizeof(itemstats_t));
        sgx_thread_mutex_unlock(&lru_locks[i]);
    }
}
...
static sgx_thread_mutex_t bump_buf_lock = SGX_THREAD_MUTEX_INITIALIZER;
...
{
    sgx_thread_mutex_lock(&cas_id_lock);
    uint64_t next_id = ++cas_id;
    sgx_thread_mutex_unlock(&cas_id_lock);
    return next_id;
}
...
item *do_item_alloc_pull(const size_t ntotal, const unsigned int id)
{
    ...
    if (i > 0)
    {
        sgx_thread_mutex_lock(&lru_locks[id]);
        itemstats[id].direct_reclaims += i;
        sgx_thread_mutex_unlock(&lru_locks[id]);
    }
...
static void item_link_q(item *it)
{
    sgx_thread_mutex_lock(&lru_locks[it->slabs_clsid]);
    do_item_link_q(it);
    sgx_thread_mutex_unlock(&lru_locks[it->slabs_clsid]);
}
...
char *item_cachedump(const unsigned int slabs_clsid, const unsigned int limit, unsigned int *bytes)
{
    ...
    while (it != NULL && (limit == 0 || shown < limit))
    {
        ...
        len = snprintf(temp, sizeof(temp), "ITEM %s [%d b; %llu s]\r\n",
                       ...
                       it->exptime == 0 ? 0 : (unsigned long long)it->exptime + process_started);
                  ...
                  struct lru_pull_tail_return *ret_it)
{
    ...
    for (; tries > 0 && search != NULL; tries--, search = next_it)
    {
        ...
        /* Expired or flushed */
        if ((search->exptime != 0 && search->exptime < current_time) || item_is_flushed(search))
        {
...
{
    sgx_thread_mutex_lock(&bump_buf_lock);
    ...
    if (bump_buf_tail == 0)
        bump_buf_tail = b;
    sgx_thread_mutex_unlock(&bump_buf_lock);
    return;
}
...
void *item_lru_bump_buf_create(void)
{
    ...
    sgx_thread_mutex_init(&b->mutex, NULL);
...
static int lru_maintainer_juggle(const int slabs_clsid)
{
    ...
    if (settings.lru_segmented)
    {
        sgx_thread_mutex_lock(&lru_locks[slabs_clsid | COLD_LRU]);
        ...
        sgx_thread_mutex_unlock(&lru_locks[slabs_clsid | COLD_LRU]);
        ...
        sgx_thread_mutex_lock(&lru_locks[slabs_clsid | HOT_LRU]);
        total_bytes += sizes_bytes[slabs_clsid | HOT_LRU];
        sgx_thread_mutex_unlock(&lru_locks[slabs_clsid | HOT_LRU]);

        sgx_thread_mutex_lock(&lru_locks[slabs_clsid | WARM_LRU]);
        total_bytes += sizes_bytes[slabs_clsid | WARM_LRU];
        sgx_thread_mutex_unlock(&lru_locks[slabs_clsid | WARM_LRU]);
    }
...
static void lru_maintainer_crawler_check(struct crawler_expired_data *cdata, logger *l)
{
    ...
    for (i = POWER_SMALLEST; i < POWER_LARGEST; i++)
    {
        ...
        {
            char *lru_name = "na";
            sgx_thread_mutex_lock(&cdata->lock);
            int x;
            ...
            sgx_thread_mutex_unlock(&cdata->lock);
        }
...
#endif
static sgx_thread_t lru_maintainer_tid;
...
static void *lru_maintainer_thread(void *arg)
{
    ...
    sgx_thread_mutex_init(&cdata->lock, NULL);
    ...
    sgx_thread_mutex_lock(&lru_maintainer_lock);
    if (settings.verbose > 2)
        fprintf(stderr, "Starting LRU maintainer background thread\n");
    while (do_run_lru_maintainer_thread)
    {
        sgx_thread_mutex_unlock(&lru_maintainer_lock);
        if (to_sleep)
            usleep(to_sleep);
        sgx_thread_mutex_lock(&lru_maintainer_lock);
