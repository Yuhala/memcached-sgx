    ...
    if (it)
    {
        ...
        for (i = KEY_TOKEN + 1; i < ntokens - 1; i++)
        {
            ...
            case 'R':
                ...
                if ((it->it_flags & ITEM_TOKEN_SENT) == 0 && !item_created && it->exptime != 0 && it->exptime < of.recache_time)
                {
                    won_token = true;
                }
                break;
...
static void process_slabs_command(conn *c, token_t *tokens, const size_t ntokens)
{
    ...
    if (ntokens == 5 && strcmp(tokens[COMMAND_TOKEN + 1].value, "reassign") == 0)
    {
        ...
        if (!(safe_strtol(tokens[2].value, (int32_t *)&src) && safe_strtol(tokens[3].value, (int32_t *)&dst)))
        {
            out_string(c, "CLIENT_ERROR bad command line format");
            return;
        }
