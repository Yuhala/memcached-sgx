                      ...
                      cache_destructor_t *destructor)
{
    ...
    if (ret == NULL || nm == NULL || ptr == NULL ||
        sgx_thread_mutex_init(&ret->mutex, NULL) == -1)
    {
        free(ret);
        free(nm);
        free(ptr);
...
{
    sgx_thread_mutex_lock(&cache->mutex);
    cache->limit = limit;
    sgx_thread_mutex_unlock(&cache->mutex);
}
...
void cache_destroy(cache_t *cache)
{
    ...
    free(cache->name);
    free(cache->ptr);
    sgx_thread_mutex_destroy(&cache->mutex);
    free(cache);
}
...
void do_cache_free(cache_t *cache, void *ptr)
{
    ...
    //pyuhala: TOFIX: I malloc all rbuffers
    return;
#ifndef NDEBUG
    /* validate redzone... */
